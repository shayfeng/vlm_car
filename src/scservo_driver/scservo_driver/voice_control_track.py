        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Google Speech API 请求失败: {e}")
                    return None
    ...
    try:
        while not node.stop_tracking:
            if node.switch_target_flag or node.current_target is None:
                # 使用语音识别获取新的目标
                voice_command = node.get_voice_command()
                
                if voice_command is None:
                    continue
                    
                if "退出" in voice_command:
                    print("\n检测到退出命令，准备结束程序...")
                    break
                    
                node.current_target = voice_command
